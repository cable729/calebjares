...
//  thanks to https://coderwall.com/p/uf2pka
  ...
  function setupHireMe() {
    ...
    $('#hire-me a').click(function() {
      activateHireSpans(400);
      return true;
    });
  }
  
  function activateHireSpans(delay) {
    $('.hire').delay(delay)
              .transition({ backgroundColor: '#ff452d' }, 1000)
              .delay(500)
              .transition({ backgroundColor: '#fff' }, 2000);
  }
  
  function setupKnobClicks() {
    $('.skill').click(function() {
      var type = $(this).data('type');
      $.smoothScroll({
        scrollTarget: '#my-timeline',
        offset: -50
      });
      $('.technologies li.' + type)
              .transition({ backgroundColor: '#63b2ff' }, 1000)
              .delay(2500)
              .transition({ backgroundColor: '#fff' }, 2000);
    });
  }
  
  function setupTechnologyClicks() {
